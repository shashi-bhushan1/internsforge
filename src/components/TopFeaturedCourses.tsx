...
const getCourseDuration = (courseName: string): string => {
  ...
  if (durationMatch) {
    ...
    if (isNotDuration) {
      ...
      // If it's Full Stack Web Development with 6 Months, keep as is
      if (name.includes('full stack web development') && extractedDuration.toLowerCase().includes('6 months')) {
  ...
  // 4 Months courses
  if (name.includes('full stack web development') && !name.includes('6 months')) {
    return '4 Months';
  }
    ...
    <section className="bg-brand-page py-8 md:py-16 px-4 sm:px-6 md:px-12 lg:pl-[145px] lg:pr-[145px] relative">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div className="p-4 space-y-2">
              {programCategories.map((category, index) => (
                <button
                  ...
                  className={`w-full lg:w-[280px] h-[44px] text-left px-4 py-3 rounded-full font-medium text-sm transition-colors flex items-center gap-3 border ${
          ...
          <div className="flex-1 grid sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              ...
              currentCourses.map((courseName, index) => {
                ...
                return (
                  <Link 
                    key={index} 
                    href={courseUrl}
                    className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow flex flex-col border-2 border-gray-200 cursor-pointer h-[380px]"
                  >
                    {/* Course Image */}
